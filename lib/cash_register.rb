class CashRegister
  ...
  def apply_discount
    if discount != 0    #if the discount is not 0, then there's a discount to apply!
      ...
      # the discount (20) needs to be converted to a float (integers are whole numbers, floats are not), then changed to a decimal (0.2).
       "After the discount, the total comes to $#{self.total}." #self.total is the new total
     else
  ...
  def void_last_transaction
    self.total=(self.total - @last_price) #the current to total we want after a voided transaction is equal to the current total minus the last transaction
